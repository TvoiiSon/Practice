...
class ScenarioRunner():
    ...
    def run(self) -> list[TestResult]:
        ...
        for scenario in self._scenarios:
            try:
                response = self._client.get(f"{self.base_url}/users/{scenario.user_id}")

                if response.status_code == scenario.expected_status:
                    logger.info(f"Сценарий '{scenario.name}' прошел успешно, статус - {response.status_code}")
                    results.append(TestResult(name=scenario.name, passed=True, actual_status=response.status_code, error=None))
                else:
                    logger.error(f"Сценарий '{scenario.name}' не прошёл: ожидали {scenario.expected_status}, получили {response.status_code}")
                    results.append(TestResult(name=scenario.name, passed=False, actual_status=response.status_code, error=f"Сценарий '{scenario.name}' не прошёл: ожидали {scenario.expected_status}, получили {response.status_code}"))
            except httpx.RequestError as e:
                logger.opt(exception=True).warning(f"Проблема с запросом для пользователя с id - {scenario.user_id}: {e}")
                results.append(TestResult(name=scenario.name, passed=False, actual_status=None, error=str(e)))
...
if __name__ == "__main__":
    with ScenarioRunner(scenarios_path="/home/TvoiiSon/Data/Work/IBS/Python/FirstProj/other/scenarios.json") as scenario:
        res = scenario.run()
